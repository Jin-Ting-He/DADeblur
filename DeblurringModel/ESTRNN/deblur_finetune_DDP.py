...
class Trainer():
    ...
    def _train_epoch(self):
        ...
        for idx, sample in enumerate(tq):
            ...
            loss = self.criterion_l2(output, sharp)
            loss.backward()
    ...
    def _valid(self, blur, sharp):
        ...
        lpips = 0#torch.mean(self.lpips_func(output.detach(), sharp.detach())).item()
    ...
    def val_save_image(self, dir_path, dataset, val_num=3):
        ...
        for idx in random.sample(range(0, len(dataset)), val_num):
            ...
            blur_middle = tensor2cv(blur_middle + 0.5)
            cv2.imwrite(save_blur_path, blur_middle)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--check_point_epoch", default=1, type=int)
    # parser.add_argument("--init_lr", default=1e-6, type=float) # 5e-4
    parser.add_argument("--init_lr", default=5e-4, type=float) # 5e-4 ## Need change
    ...
    parser.add_argument("--val_save_epochs", default=10, type=int)
    ...
    if args.resume:
        ...
        load_model_state = torch.load(args.resume)
        ...
        if 'state_dict' in load_model_state.keys():
            ...
            model_load = judge_and_remove_module_dict(load_model_state)
            net.load_state_dict(model_load)
        os.makedirs(args.dir_path, exist_ok=True)
    else:
        os.makedirs(args.dir_path, exist_ok=True)

    print("device list: ",args.local_rank)
    # Model
    ...
    # Train_set = Video_GoPro_Loader(data_path=train_data_path, mode="train", crop_size=args.crop_size)
    # Train_set = Video_BSD_Loader(data_path=train_data_path, mode="train", crop_size=args.crop_size)
    Train_set = Video_BSD_json_Loader(data_path=train_data_path, mode="train", crop_size=args.crop_size)    
    ...
    # Val loader
    # Val_set = Video_GoPro_Loader(data_path=args.data_path, mode="test", crop_size=args.crop_size)
    Val_set = Video_BSD_all_valid_Loader(data_path=args.valid_data_path, mode="test", crop_size=args.crop_size)
    ...
    if dist.get_rank() == 0:
        ...
        writer = SummaryWriter(os.path.join("home/jthe/Deblur_Domain_Adaptation/ESTRNN/ESTRNN_log", args.model_name))
