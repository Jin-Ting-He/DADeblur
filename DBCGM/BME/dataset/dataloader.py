...
class BlurMagDataset(Dataset):
    def __init__(self,dataset_root,train=True):
        ...
        if train:
            ...
            for video in video_list:
                ...
                file_list = os.listdir(os.path.join(dataset_root, video, 'Blur/RGB'))
                for file in file_list:
                    self.blur_img_path_list.append(os.path.join(dataset_root, video, 'Blur/RGB', file))
    ...
    def __getitem__(self,idx):
        if self.train:
            ...
            video_name = self.blur_img_path_list[idx].split('/')[-4]
