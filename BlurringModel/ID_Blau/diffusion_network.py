...
from ID_Blau.models.losses import CharbonnierLoss
...
class DDPM(nn.Module):
    ...
    @torch.no_grad()
    def sample_grid(self, condition, process_step, device):
        b, c, h, w = condition.shape
        x = torch.randn((b, 3, h, w), device=device)
        ...
        process_x = torch.stack(process_x, dim=0).view(-1, 3, h, w)
...
class DDIM(DDPM):
    ...
    @torch.no_grad()
    def sample_grid(self, condition, process_step, device, sample_timesteps, ddim_eta=0.0):
        b, c, h, w = condition.shape
        x = torch.randn((b, 3, h, w), device=device)
        process_x = [x]
        interval = sample_timesteps / (process_step - 1)
        ...
        step = 1
        for i in tqdm(reversed(range(0, sample_timesteps)), desc='sampling loop time step', total=sample_timesteps):
            t_batch = torch.tensor(
                [ddim_timestep_seq[i]], device=device, dtype=torch.long).repeat(b)
            prev_t_batch = torch.tensor(
                [ddim_timestep_prev_seq[i]], device=device, dtype=torch.long).repeat(b)
